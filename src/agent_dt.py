...
# ============================================================================
# Tool Functions (Nodes in the graph)
# ============================================================================

class ProductRecommendationAgent:
    """LangGraph-based product recommendation agent using ReAct pattern"""
    ...
    def _build_graph(self) -> StateGraph:
        ...
        graph.add_node("check_info", self._check_info)
        graph.add_node("ask_clarification", self._ask_clarification)
        graph.add_node("search_products", self._search_products)
        graph.add_node("filter_rank", self._filter_rank)
        graph.add_node("explain_recommendation", self._explain_recommendation)
        graph.add_node("return_result", self._return_result)
        
        # Add edges
        graph.add_edge(START, "understand_query")
        graph.add_edge("understand_query", "check_info")
        
        # Conditional edge: if needs clarification, ask; otherwise continue
        graph.add_conditional_edges(
            "check_info",
            lambda state: "ask_clarification" if state.needs_clarification else "search_products",
            {
        ...
        graph.add_edge("ask_clarification", "check_info")  # Loop back to check again
        graph.add_edge("search_products", "filter_rank")
        graph.add_edge("filter_rank", "explain_recommendation")
        graph.add_edge("explain_recommendation", "return_result")
        graph.add_edge("return_result", END)
        
        return graph.compile()
    
    def _understand_query(self, state: AgentState) -> AgentState:
        """Node 1: Parse and extract query information"""
    ...
    def _check_info(self, state: AgentState) -> AgentState:
        """Node 2: Check if we have enough information"""
    ...
    def _ask_clarification(self, state: AgentState) -> AgentState:
        """Node 3: Ask user for missing information"""
        ...
        # Simulate user providing clarification (in real scenario, this would come from user)
        # For now, we'll assume we got info
    ...
    def _search_products(self, state: AgentState) -> AgentState:
        """Node 4: Search products matching criteria"""
        ...
        state.history.append({
            ...
            "products": [p.dict() for p in state.products[:5]]  # Log first 5
        })
        
        return state
    
    def _filter_rank(self, state: AgentState) -> AgentState:
        """Node 5: Filter and rank products"""
    ...
    def _explain_recommendation(self, state: AgentState) -> AgentState:
        """Node 6: Generate explanation for recommendations"""
...
Highlight unique features and value propositions."""
    ...
    def _return_result(self, state: AgentState) -> AgentState:
        """Node 7: Format and return final result"""
...
"""
    ...
    def run(self, user_input: str) -> Dict[str, Any]:
        ...
        # LangGraph returns a dict-like state after invocation
        ...
        for p in recommendations:
            if hasattr(p, "dict"):
                recommendations_data.append(p.dict())
            else:
                recommendations_data.append(p)
